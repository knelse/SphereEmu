...
internal static class Program
{
        ...
        foreach (var rawLine in File.ReadLines(options.InputTsvPath))
        {
            ...
            try
            {
                ...
                var prefixedName = CollapseDuplicateNpcPrefix(
                    $"NPC_{FileFormatCulture.FormatInt(row.Id)}_" + displayNameLatin);
                ...
                sb.Append("\" unique_id=");
                sb.Append(FileFormatCulture.FormatInt(Random.Shared.Next(100_000_000, int.MaxValue)));
                sb.Append(" instance=ExtResource(\"");
                sb.Append(NpcExtResource);
                sb.AppendLine("\")]");
                AppendTransform3DWithYRotation(sb, gx, gy, gz, row.Angle);
                sb.Append("NameID = ");
                sb.AppendLine(FileFormatCulture.FormatInt(4000 + row.NameId));
                sb.Append("ModelName = ");
                sb.AppendLine(FormatTscnStringValue(row.ModelName));
                sb.Append("IconName = ");
                sb.AppendLine(FormatTscnStringValue(row.IconName));
                sb.Append("NpcType = ");
                sb.AppendLine(FileFormatCulture.FormatInt((int)row.NpcType));
                sb.AppendLine("VendorItemTierMax = 15");
                sb.AppendLine("VendorLocation = 0");
                sb.Append("Angle = ");
                sb.AppendLine(FileFormatCulture.FormatInt(row.Angle));
                sb.Append("ID = ");
                sb.AppendLine(FileFormatCulture.FormatInt(row.Id));
                sb.Append("ObjectType = ");
                sb.AppendLine(FileFormatCulture.FormatInt((ushort)row.ObjectType));
            }
            catch (Exception ex)
            {
    ...
    private static string InsertNpcRootNode(string sceneText, string nodeName)
    {
        ...
        var uniqueId = FileFormatCulture.FormatInt(Random.Shared.Next(100_000_000, int.MaxValue));
        var escaped = EscapeTscnString(nodeName);
    ...
    private static string FormatFloat(float f) =>
        FileFormatCulture.FormatFloat(f, "G");
    ...
    private sealed record NpcSpawnRow
    {
        ...
        public static NpcSpawnRow Parse(string[] p)
        {
            ...
            var angle = (int)FileFormatCulture.ParseFloat(p[6]);
            var nameId = FileFormatCulture.ParseInt(p[7]);
            var modelLen = FileFormatCulture.ParseInt(p[8]);
            var modelName = p[9];
            var iconLen = FileFormatCulture.ParseInt(p[10]);
            var iconName = p[11];
            var npcTypeRaw = FileFormatCulture.ParseInt(p[12]);
        ...
        private static int ParseHexInt32(string s)
        {
            ...
            return FileFormatCulture.ParseHexInt(s);
        }
    }
