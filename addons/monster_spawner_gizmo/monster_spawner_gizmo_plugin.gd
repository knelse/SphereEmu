...
const SPAWNER_SCRIPT_PATH := "res://Godot/Scripts/Objects/HelperGizmos/MonsterSpawner.cs"

var _spawner_script: Script


func _init() -> void:
	create_material("spawn_ok", Color(0.25, 0.95, 0.35, 0.9), false, true)
	create_material("spawn_error", Color(0.95, 0.28, 0.22, 0.95), false, true)
	create_material("slot", Color(0.95, 0.15, 0.12, 0.98), false, true)
	create_material("leash", Color(0.45, 0.6, 1.0, 0.35), false, true)
	_spawner_script = load(SPAWNER_SCRIPT_PATH) as Script


func _has_gizmo(node: Node3D) -> bool:
	return _is_monster_spawner(node)


func _redraw(gizmo: EditorNode3DGizmo) -> void:
	var node := gizmo.get_node_3d()
	if not _is_monster_spawner(node):
		return

	gizmo.clear()

	var has_error := bool(node.get("HasSpawnError"))
	var spawn_radius := float(node.get("SpawnRadiusMeters"))
	var leash_radius := float(node.get("LeashRadiusMeters"))
	...
	if leash_radius > spawn_radius + 0.05:
		gizmo.add_lines(_build_horizontal_circle(leash_radius), get_material("leash", gizmo), false)
...
func _is_monster_spawner(node: Node3D) -> bool:
