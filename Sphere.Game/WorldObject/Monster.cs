...
public partial class Monster : WorldObject
{
	...
	private int GetMonsterLevel() => MonsterInstance?.Level ?? _level;
	...
	{
		var level = GetMonsterLevel();
		var currentHp = MonsterInstance?.CurrentHp ?? 50;
		var maxHp = MonsterInstance?.MaxHp ?? 50;

		UpdateMonsterHpFields(packetParts, currentHp, maxHp, level <= 1);
		UpdateMonsterLevelFields(packetParts, level);
