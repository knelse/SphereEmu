...
public class Client : Node
{
    ...
    public static readonly int BUFSIZE = 1024;
    ...
    public override void _Ready()
    {
        ...
        while (streamPeer.GetBytes(rcvBuffer) == 0)
        {
        }
        ...
        while (streamPeer.GetBytes(rcvBuffer) <= 12)
        {
        }
        ...
        var selectedCharacterIndex =  CharacterScreenCreateDeleteSelect(charListData);
        ...
        streamPeer.PutPartialData(selectedCharacter!.ToGameDataByteArray());
        // Interlocked.Increment(ref playerCount);
        
        while (streamPeer.GetBytes(rcvBuffer) == 0x13)
        {
        }
        ...
        MoveToNewPlayerDungeon(selectedCharacter);
    }

    private int CharacterScreenCreateDeleteSelect(ClientInitialData charListData)
    {
        while (streamPeer.GetBytes(rcvBuffer) != 0x15)
        {
            if (rcvBuffer[0] == 0x2a)
            {
            ...
            if (rcvBuffer[0] < 0x1b || (rcvBuffer[13] != 0x08 || rcvBuffer[14] != 0x40 || rcvBuffer[15] != 0x80 || rcvBuffer[16] != 0x05))
            {
                continue;
            }
    ...
    private void MoveToNewPlayerDungeon(CharacterData selectedCharacter)
    {
        Task.Run(() =>
        {
            // while (ns.CanRead)
            ...
            streamPeer.PutPartialData(
                ...
                selectedCharacter.GetTeleportAndUpdateCharacterByteArray(
                    new WorldCoords(-1098.69506835937500, 4501.61474609375000, 1900.05493164062500, 1.57079637050629),
