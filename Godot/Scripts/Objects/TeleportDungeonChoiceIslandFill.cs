...
public partial class TeleportDungeonChoiceIslandFill : Node3D
{
	...
	[Export]
	public string TeleportDungeonChoiceIslandDataFilePath { get; set; } = @"d:\SphereDev\_sphereDumps\teleport_dungeon_choice_island.txt";

	[Export]
	public string TeleportDungeonChoiceIslandScenePath { get; set; } = "res://Godot/Scenes/teleport_dungeon_choice_island.tscn";
	...
	public void RebuildTeleportDungeonChoiceIsland()
	{
		...
		if (!WorldObjectDumpFillCommon.TryLoadPackedScene(TeleportDungeonChoiceIslandScenePath, "TeleportDungeonChoiceIslandFill", out var scene))
		{
			return;
		}

		if (!WorldObjectDumpFillCommon.TryReadTextFile(TeleportDungeonChoiceIslandDataFilePath, "TeleportDungeonChoiceIslandFill", out var text))
		{
			return;
		}
		...
		foreach (var (lineNumber, parts) in WorldObjectDumpFillCommon.EnumerateDataLinesBottomUp(text))
		{
			...
			if (!WorldObjectDumpFillCommon.MatchesTypeTokenIfPresent(parts, typeIndex: 1, expectedTypeValue: TeleportDungeonChoiceIslandTypeValue))
			{
